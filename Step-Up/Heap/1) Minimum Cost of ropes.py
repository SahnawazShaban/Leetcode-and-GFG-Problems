...
class Solution:
    ...
    def minCost(self, arr, n):
        ...
        while len(arr) > 1:
            ...
            # Calculate the sum and add it to the total
            min_sum = top1 + top2
            total += min_sum
...
The heapify operation takes O(n) time, where n is the length of the input array arr.
In the while loop, we perform extract-min (heappop) and insert (heappush) operations. Each iteration of the loop takes O(log n) time, as the heap size is reduced by half in each iteration.
The loop runs until the heap has only one element left, so there are at most n-1 iterations.
Therefore, the overall time complexity is O(n + (n-1) * log n), which simplifies to O(n log n).

Space Complexity:
The space complexity is determined by the heap data structure. The heapify operation is performed in-place and does not require additional space. However, the heap itself requires O(n) space to store the elements.
'''

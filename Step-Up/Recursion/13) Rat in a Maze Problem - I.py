...
class Solution:
    ...
    # Solution - 2 : each if statement combine into loop
    
    def solve(self, i, j, m, n, ans, vis, move_dir, di, dj):
        if i == n-1 and j == n-1:
            ans.append(move_dir)
            return 
        ...
        for idx in range(4):
            ...
            if 0 <= nexti < n and 0 <= nextj < n and not vis[nexti][nextj] and m[nexti][nextj] == 1:
                ...
                self.solve(nexti, nextj, m, n, ans, vis, move_dir + direction[idx], di, dj)
                
                vis[i][j] = 0
        
    
    # m is matrix
    # n is length of a matrix
    def findPath(self, m, n):
        vis = [[0 for _ in range(n)] for _ in range(n)]
        di = [1,0,0,-1]
        dj = [0,-1,1,0]
